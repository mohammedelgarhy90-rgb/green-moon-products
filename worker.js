...
export default {
    async fetch(request, env) {
        try {
            ...
            // Migrate older D1 schemas safely. CREATE TABLE IF NOT EXISTS does not add
            // columns to tables that already existed, so older deployments can otherwise
            // fail on UPDATE statements that reference updated_at.
            const db = getDB(env);
            async function ensureColumn(table, column, definition) {
                try {
                    const info = await db.prepare(`PRAGMA table_info(${table})`).all();
                    const exists = (info.results || []).some(r => String(r.name) === column);
                    if (!exists) await db.prepare(`ALTER TABLE ${table} ADD COLUMN ${definition}`).run();
                } catch (_) {}
            }
            await ensureColumn("settings", "updated_at", "updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP");
            await ensureColumn("cms_content", "updated_at", "updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP");
            await ensureColumn("articles", "updated_at", "updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP");
            await ensureColumn("articles", "created_at", "created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP");
            await ensureColumn("articles", "active", "active INTEGER NOT NULL DEFAULT 1");
            await ensureColumn("articles", "sort_order", "sort_order INTEGER NOT NULL DEFAULT 0");
            await ensureColumn("menu_items", "updated_at", "updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP");
            await ensureColumn("menu_items", "created_at", "created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP");
            await ensureColumn("menu_items", "active", "active INTEGER NOT NULL DEFAULT 1");
            await ensureColumn("menu_items", "sort_order", "sort_order INTEGER NOT NULL DEFAULT 0");
            await ensureColumn("products", "delivery", "delivery REAL NOT NULL DEFAULT 0");
            await ensureColumn("products", "max_qty", "max_qty INTEGER NOT NULL DEFAULT 99");
            await ensureColumn("products", "wholesale_price", "wholesale_price REAL NOT NULL DEFAULT 0");
            await ensureColumn("products", "cost_price", "cost_price REAL NOT NULL DEFAULT 0");
            if (p === "/admin" || p === "/admin/") {
